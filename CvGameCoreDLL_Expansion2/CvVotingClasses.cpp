...
int CvVoterDecision::GetPercentContributionToOutcome(PlayerTypes eVoter, int iChoice, bool bChangeHost, int& iPercentOfPlayerVotes)
{
	...
	for (PlayerVoteList::iterator it = m_vVotes.begin(); it != m_vVotes.end(); ++it)
	{
		...
		else if (!bChangeHost && it->ePlayer == eVoter && it->iChoice != LeagueHelpers::CHOICE_NONE)
		{
			iVotes -= it->iNumVotes;
		}
	}
...
int CvVoterDecision::GetPercentContributionAgainstOutcome(PlayerTypes eVoter, int iChoice, int& iPercentOfPlayerVotes)
{
	...
	for (PlayerVoteList::iterator it = m_vVotes.begin(); it != m_vVotes.end(); ++it)
	{
		if (it->iChoice != iChoice && it->iChoice != LeagueHelpers::CHOICE_NONE)
		{
			iTotalVotes += it->iNumVotes;
		...
		else if (it->ePlayer == eVoter && it->iChoice == iChoice)
		{
			iVotes -= it->iNumVotes;
		}
	}
...
void CvLeague::DoTurn(LeagueSpecialSessionTypes eTriggeredSpecialSession)
{
	...
	else if (!IsInSession())
	{
		...
		else if (GetTurnsUntilSession() == /*5*/ GD_INT_GET(LEAGUE_SESSION_SOON_WARNING_TURNS))
		{
...
int CvLeague::GetSessionTurnInterval()
{
	...
	int iInterval = /*12*/ GD_INT_GET(LEAGUE_SESSION_INTERVAL_BASE_TURNS);
...
void CvLeague::DoProposeEnact(ResolutionTypes eResolution, PlayerTypes eProposer, int iChoice)
{
	...
	if (eProposer != NO_PLAYER)
	{
		...
		DoEnactProposalDiplomacy(eResolution, eProposer, iChoice);
	}
	...
	for (ActiveResolutionList::iterator it = m_vActiveResolutions.begin(); it != m_vActiveResolutions.end(); ++it)
	{
		if (it->GetID() == iResolutionID)
		{
			if (eProposer != NO_PLAYER)
			{
				...
				// Proposals made by players could affect Diplomacy AI
				DoRepealProposalDiplomacy(iResolutionID, eProposer);
			}
...
void CvLeague::DoEnactProposalDiplomacy(ResolutionTypes eResolution, PlayerTypes eProposer, int iProposerChoice)
{
	for (MemberList::iterator it = m_vMembers.begin(); it != m_vMembers.end(); ++it)
	{
...
void CvLeague::DoRepealProposalDiplomacy(int iTargetResolutionID, PlayerTypes eProposer)
{
	for (MemberList::iterator it = m_vMembers.begin(); it != m_vMembers.end(); ++it)
	{
		if (it->ePlayer != eProposer && CanEverVote(it->ePlayer) && GET_PLAYER(it->ePlayer).isAlive())
...
CvString CvLeague::GetLeagueSplashDescription(LeagueSpecialSessionTypes eGoverningSpecialSession, bool bJustFounded)
{
	...
	if (!bJustFounded && pInfo != NULL)
	{
		//don't show theh era advance text if this session was triggered by a building
		if (pInfo->GetBuildingTrigger() == NO_BUILDING)
		{
...
void CvLeague::CheckStartSpecialSession(LeagueSpecialSessionTypes eSpecialSession)
{
	if (CanStartSpecialSession(eSpecialSession))
	{
		...
		if (pInfo != NULL)
		{
			...
			DLLUI->AddMessage(0, kGame.getActivePlayer(), false, /*10*/ GD_INT_GET(EVENT_MESSAGE_TIME), 
...
void CvLeague::FinishSession()
{
	...
	// Resolve Repeal Proposals on table
	for (RepealProposalList::iterator it = m_vRepealProposals.begin(); it != m_vRepealProposals.end(); ++it)
	{
		...
		if (it->IsPassed(iTotalSessionVotes))
		{
			if (eProposer != NO_PLAYER)
			{
				// First let's go through the players who voted for it to pass
				for (std::vector<PlayerTypes>::iterator playerIt = vHelpedOutcome.begin(); playerIt != vHelpedOutcome.end(); playerIt++)
				{
					int iPercentOfPlayerVotes = 0;
					int iVotePercent = it->GetRepealDecision()->GetPercentContributionToOutcome(*playerIt, it->GetRepealDecision()->GetDecision(), false, iPercentOfPlayerVotes);
					...
					if (iVotePercent > 0)
					{
						...
						for (int iPlayerLoop = 0; iPlayerLoop < MAX_MAJOR_CIVS; iPlayerLoop++)
						{
							...
							int iScoreChange = ((iPlayerVoteChange + iOutcomeVoteChange) * iDesireMultiplier) / 100;
							GET_PLAYER(eLoopPlayer).GetDiplomacyAI()->ChangeVotingHistoryScore(*playerIt, iScoreChange);
						}

						// Proposer is grateful to other people who helped it pass
				...
				// Next let's go through the players who voted for it to fail!
				for (std::vector<PlayerTypes>::iterator playerIt = vHarmedOutcome.begin(); playerIt != vHarmedOutcome.end(); playerIt++)
				{
					int iPercentOfPlayerVotes = 0;
					int iVotePercent = it->GetRepealDecision()->GetPercentContributionAgainstOutcome(*playerIt, it->GetRepealDecision()->GetDecision(), iPercentOfPlayerVotes);
					...
					if (iVotePercent > 0)
					{
						...
						for (int iPlayerLoop = 0; iPlayerLoop < MAX_MAJOR_CIVS; iPlayerLoop++)
						{
							...
							int iOutcomeVoteChange = (iVotePercent * /*200*/ GD_INT_GET(VOTING_HISTORY_SCORE_OUTCOME_VOTE_PERCENT_VALUE)) / 100;
							int iScoreChange = ((iPlayerVoteChange + iOutcomeVoteChange) * iDesireMultiplier) / 100;
							GET_PLAYER(eLoopPlayer).GetDiplomacyAI()->ChangeVotingHistoryScore(*playerIt, iScoreChange * -1);
						}
					}
				}
		...
		// This Repeal Proposal failed.
		else if (eProposer != NO_PLAYER)
		{
			// First let's go through the players who voted for it to fail
			for (std::vector<PlayerTypes>::iterator playerIt = vHelpedOutcome.begin(); playerIt != vHelpedOutcome.end(); playerIt++)
			{
				int iPercentOfPlayerVotes = 0;
				int iVotePercent = it->GetRepealDecision()->GetPercentContributionToOutcome(*playerIt, it->GetRepealDecision()->GetDecision(), false, iPercentOfPlayerVotes);
			...
			// Next let's go through the players who voted for it to pass!
			for (std::vector<PlayerTypes>::iterator playerIt = vHarmedOutcome.begin(); playerIt != vHarmedOutcome.end(); playerIt++)
			{
				int iPercentOfPlayerVotes = 0;
				int iVotePercent = it->GetRepealDecision()->GetPercentContributionAgainstOutcome(*playerIt, it->GetRepealDecision()->GetDecision(), iPercentOfPlayerVotes);
	...
	// Resolve Enact Proposals on table
	for (EnactProposalList::iterator it = m_vEnactProposals.begin(); it != m_vEnactProposals.end(); ++it)
	{
		if (eRecurringProposal != NO_RESOLUTION && eRecurringProposal == it->GetType())
		{
		...
		if (it->IsPassed(iTotalSessionVotes))
		{
			if (eProposer != NO_PLAYER)
			{
				...
				{
					int iPercentOfPlayerVotes = 0;
					int iVotePercent = it->GetVoterDecision()->GetPercentContributionToOutcome(*playerIt, it->GetVoterDecision()->GetDecision(), false, iPercentOfPlayerVotes);
				...
				{
					int iPercentOfPlayerVotes = 0;
					int iVotePercent = it->GetVoterDecision()->GetPercentContributionAgainstOutcome(*playerIt, it->GetVoterDecision()->GetDecision(), iPercentOfPlayerVotes);
			...
			if (it->GetEffects()->bChangeLeagueHost && !vHelpedOutcome.empty() && eNewHost != NO_PLAYER)
			{
				for (std::vector<PlayerTypes>::iterator playerIt = vHelpedOutcome.begin(); playerIt != vHelpedOutcome.end(); ++playerIt)
				{
					...
					int iVotePercent = it->GetVoterDecision()->GetPercentContributionToOutcome(*playerIt, it->GetVoterDecision()->GetDecision(), true, iUselessReferenceVariable);
...
void CvLeague::AssignProposalPrivileges()
{
	...
	for (int i = 0; i < vpPossibleProposers.size(); i++)
	{
		...
		if (!vpPossibleProposers.GetElement(i)->bMayPropose)
		{
			...
			vpPossibleProposers.GetElement(i)->iProposals = /*1*/ GD_INT_GET(LEAGUE_MEMBER_PROPOSALS_BASE);
			iPrivileges--;
		}
	}
...
void CvLeague::NotifyProposalResult(CvEnactProposal* pProposal)
{
	...
	if (pProposal->GetEffects()->bDiplomaticVictory)
	{
		...
		else
		{
			...
			sTemp << GC.getGame().GetVotesNeededForDiploVictory() << /*2 in CP, 1 in CSD*/ GD_INT_GET(LEAGUE_NUM_LEADERS_FOR_EXTRA_VOTES);
			sMessage += sTemp.toUTF8();
		}
...
void CvGameLeagues::DoTurn()
{
	if (!GC.getGame().isOption(GAMEOPTION_NO_LEAGUES))
	{
		...
		else
		{
			...
			LeagueSpecialSessionTypes eLastSpecialSession = pLeague->GetLastSpecialSession();
			
			for (int i = 0; i < GC.getNumLeagueSpecialSessionInfos(); i++)
			{
				if ((LeagueSpecialSessionTypes)i == eLastSpecialSession)
					continue;
		...
		if (GetNumActiveLeagues() == 0)
		{
			...
			for (int i = 0; i < GC.getNumLeagueSpecialSessionInfos(); i++)
			{
				...
				if (pInfo != NULL)
				{
					...
					if (eBuildingTrigger != NO_BUILDING && LeagueHelpers::IsBuildingForTriggerBuiltAnywhere(eBuildingTrigger) && eEraTrigger <= (int)LeagueHelpers::GetGameEraForTrigger())
					{
						eGoverningSpecialSession = e;
						bBeginAsUnitedNations = pInfo->IsUnitedNations();
						break;
					}
					else if (eEraTrigger == eGoverningEraTrigger)
					{
						eGoverningSpecialSession = e;
						bBeginAsUnitedNations = pInfo->IsUnitedNations();
						break;
			...
			DLLUI->AddMessage(0, GC.getGame().getActivePlayer(), false, /*10*/ GD_INT_GET(EVENT_MESSAGE_TIME), 
			...
			if (GetPlayer()->isHuman(ISHUMAN_AI_WORLD_CONGRESS))
			{
				...
				for (EnactProposalList::iterator it = vEnactProposals.begin(); it != vEnactProposals.end(); ++it)
				{
					...
					if (it->GetVoterDecision()->GetType() == RESOLUTION_DECISION_YES_OR_NO)
					{
						...
						{
							DesireLevels eDesire = EvaluateDesire(ScoreVoteChoiceYesNo(&(*it), LeagueHelpers::CHOICE_YES, /*bEnact*/ true, /*bConsiderGlobal*/ false));
				...
				for (RepealProposalList::iterator it = vRepealProposals.begin(); it != vRepealProposals.end(); ++it)
				{
					...
					if (it->GetRepealDecision()->GetType() == RESOLUTION_DECISION_REPEAL)
					{
						...
						{
							DesireLevels eDesire = EvaluateDesire(ScoreVoteChoiceYesNo(&(*it), LeagueHelpers::CHOICE_YES, /*bEnact*/ false, /*bConsiderGlobal*/ false));
							if (eDesire > DESIRE_NEUTRAL)
								iDesiredChoice = LeagueHelpers::CHOICE_YES;
							else if (eDesire < DESIRE_NEUTRAL)
								iDesiredChoice = LeagueHelpers::CHOICE_NO;
...
/// How much do we like the outcome of the vote on a resolution? Used to update voting history score for Diplo AI.
CvLeagueAI::DesireLevels CvLeagueAI::EvaluateDesireForVoteOutcome(CvProposal* pProposal, int iChoice, bool bEnact)
{
	return EvaluateDesire(ScoreVoteChoiceYesNo(pProposal, iChoice, bEnact, /*bConsiderGlobal*/ false));
}
...
int CvLeagueAI::ScoreVoteChoiceYesNo(CvProposal* pProposal, int iChoice, bool bEnact, bool bConsiderGlobal, bool bForSelf, bool bProposingToEnact)
{
	...
	if (eProject != NO_LEAGUE_PROJECT)
	{
		...
		bool bCanBronze = iProductionMightPercent >= 25 || iOurProductionMight * 100 > GD_FLOAT_GET(LEAGUE_PROJECT_REWARD_TIER_1_THRESHOLD) * iProjectCost;

		// Project specific benefits
		if (eWorldsFair == eProject)
		{
			if (bCanGold)
			{
				// factories
				iExtra += (iPercentofWinning * 30 * GetPlayer()->GetNumEffectiveCities()) / 100;
				// golden age
		...
		else if (eWargames == eProject)
		{
			if (bCanGold)
			{
				...
				iExtra += ( iPercentofWinning * iImprovements) / 4;
				iExtra += 2 * iPercentofWinning;
			}
			if (bCanSilver)
			{
	...
	if (pProposal->GetEffects()->bEmbargoCityStates)
	{
		...
		for (int i = 0; i < MAX_CIV_PLAYERS; i++)
		{
			...
			if (e != GetPlayer()->GetID() && GET_PLAYER(e).isAlive())
			{
				...
				else if (pLeague && !pLeague->IsTradeEmbargoed(GetPlayer()->GetID(), e))
				{
	...
	if (pProposal->GetEffects()->bEmbargoPlayer)
	{
		...
		if (eTargetPlayer == GetPlayer()->GetID())
		{
			...
			for (int iPlayerLoop = 0; iPlayerLoop < MAX_MAJOR_CIVS; iPlayerLoop++)
			{
				...
				if (eLoopPlayer != GetPlayer()->GetID() && GET_PLAYER(eLoopPlayer).isAlive() && GET_PLAYER(eLoopPlayer).isMajorCiv() && GET_TEAM(GET_PLAYER(eLoopPlayer).getTeam()).GetMaster() != GetPlayer()->getTeam())
				{
					...
					iTradeDealValue += min(400, GC.getGame().GetGameDeals().GetDealValueWithPlayer(GetPlayer()->GetID(), eLoopPlayer, true, false) / 10);
		...
		else if (GET_PLAYER(eTargetPlayer).isMajorCiv())
		{
			if (bForSelf)
			{
				if (GetPlayer()->GetDiplomacyAI()->GetCivApproach(eTargetPlayer) < CIV_APPROACH_GUARDED)
				{
				...
				iExtra -= (GetPlayer()->GetDiplomacyAI()->GetCivOpinion(eTargetPlayer) - CIV_OPINION_COMPETITOR) * 100;
				...
				iTradeDealValue += min(500,GC.getGame().GetGameDeals().GetDealValueWithPlayer(GetPlayer()->GetID(), eTargetPlayer, true, false) / 5);
	...
	if (pProposal->GetEffects()->iMemberDiscoveredTechMod != 0)
	{
		...
		for (int iTeamLoop = 0; iTeamLoop < MAX_MAJOR_CIVS; iTeamLoop++)	// Looping over all MAJOR teams
		{
			...
			if (GET_TEAM(eTeam).isAlive())
			{
				...
				{
					iTotalTechs += iTechs;
					if (iTechs > iMostTechsKnown)
					{
						iMostTechsKnown = iTechs;
					}
					if (eTeam == GetPlayer()->getTeam())
					{
						iOurTechs = iTechs;
	...
	if (pProposal->GetEffects()->bNoTrainingNuclearWeapons)
	{
		...
		for (int iMajorLoop = 0; iMajorLoop < MAX_MAJOR_CIVS; iMajorLoop++)
		{
			...
			if (GET_PLAYER(eMajor).isAlive() && GET_PLAYER(eMajor).isMajorCiv())
			{
				...
				{
					if (GetPlayer()->GetDiplomacyAI()->GetCivApproach(eMajor) < CIV_APPROACH_DECEPTIVE)
					{
		...
		pProposal->GetEffects()->iReligionSpreadStrengthMod != 0)
	{
		...
		if (eFoundedReligion != NO_RELIGION)		// if this player founded a religion
		{
			...
			{
				iExtra += 150 * iPossibleExtraVotes;
			}
			else
			{
				if (bForSelf)
				{
					iExtra -= 150 * iPossibleExtraVotes;		// negative because other world religion blocks our own
				}
			}
			if (!bDiploVictoryEnabled)
			{
		...
		pProposal->GetEffects()->iOtherIdeologyRebellionMod != 0)
	{
		...
		if (eOurIdeology != NO_POLICY_BRANCH_TYPE)
		{
			...
			{
				iExtra += iPotentialExtraVotes * 200 + iIdeologyPressure * 100;
			}
			else
			{
				if (bForSelf)
		...
		pProposal->GetEffects()->iLandmarkCulture != 0)
	{
		...
		iExtra += 40 * iNumGPImprovements + 50 * iNumLandmarks;
	...
	if (pProposal->GetEffects()->bOpenDoor)
	{
		...
		if (eTargetCityState != NO_PLAYER && GET_PLAYER(eTargetCityState).isMinorCiv())
		{
			...
			int iAllyDesire = (eApproach - CIV_APPROACH_GUARDED) * 150;
			PlayerProximityTypes eProximity = GET_PLAYER(eTargetCityState).GetProximityToPlayer(ePlayer);
			int iProximity = 0;
			switch (eProximity)
			{
			...
			if (GET_PLAYER(eLoopPlayer).isMajorCiv() && GET_PLAYER(eLoopPlayer).isAlive() && eLoopPlayer != GetPlayer()->GetID())
			{
				...
				if (eOtherIdeology != NO_POLICY_BRANCH_TYPE)
				{
					...
					if ((eOtherIdeology != ePlayerIdeology) && !GET_PLAYER(eLoopPlayer).GetDiplomacyAI()->IsVassal(GetPlayer()->GetID()))
					{
						...
						{
							iExtra -= 75 * (eOpinion - CIV_OPINION_NEUTRAL);
						}
					}
					else
					{
...
int CvLeagueAI::ScoreVoteChoicePlayer(CvProposal* pProposal, int iChoice, bool bEnact)
{
	...
	if (pProposal->GetEffects()->bDiplomaticVictory)
	{
		...
		case ALIGNMENT_TEAMMATE:
		case ALIGNMENT_LIBERATOR:
			iScore += /*1000*/ GD_INT_GET(AI_WORLD_LEADER_BASE_WEIGHT_SELF) + 30 * iTheirPercent;
			break;
		case ALIGNMENT_ALLY:
			if (bIsLiked)
			{
