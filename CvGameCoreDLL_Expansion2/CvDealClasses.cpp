...
{
	PRECONDITION(ePlayer < MAX_CIV_PLAYERS, "DEAL: Invalid Player Index.");
...
{
	PRECONDITION(eFromPlayer >= 0, "DEAL: Invalid Player Index.");
	PRECONDITION(eFromPlayer < MAX_CIV_PLAYERS, "DEAL: Invalid Player Index.");
...
bool CvDeal::IsPossibleToTradeItem(PlayerTypes ePlayer, PlayerTypes eToPlayer, TradeableItems eItem, int iData1, int iData2, int iData3, bool bFlag1, bool bFinalizing)
{
		...
		if (bPeaceDeal)
		{
			if (eItem != TRADE_ITEM_PEACE_TREATY && eItem != TRADE_ITEM_THIRD_PARTY_PEACE)
			{
				if (bOneSided && this->GetSurrenderingPlayer() != ePlayer)
					return false;
				else if (!bOneSided && !pFromPlayer->isHuman(ISHUMAN_AI_DIPLOMACY))
					return false;
			}
		}
	...
	CvDeal* pRenewDeal = GC.getGame().GetGameDeals().GetRenewDeal(ePlayer, eToPlayer);
	...
	case TRADE_ITEM_GOLD_PER_TURN:
		{
			...
			if (pRenewDeal)
			{
				...
				iGoldRate -= pRenewDeal->GetGoldPerTurnTrade(eToPlayer);
			}
	...
	case TRADE_ITEM_RESOURCES:
		{
			...
			if (iData2 != -1)
			{
				...
				if (pRenewDeal)
				{
					int iResourcesAlreadyInDealFrom = pRenewDeal->GetNumResourcesInDeal(ePlayer, eResource);
					if (iResourcesAlreadyInDealFrom > 0)
					{
						iNumAvailableToUs += iResourcesAlreadyInDealFrom;
						iNumAvailableToOther -= iResourcesAlreadyInDealFrom;
					}
					int iResourcesAlreadyInDealTo = pRenewDeal->GetNumResourcesInDeal(eToPlayer, eResource);
					if (iResourcesAlreadyInDealTo > 0)
					{
						iNumAvailableToUs -= iResourcesAlreadyInDealTo;
						iNumAvailableToOther += iResourcesAlreadyInDealTo;
					}
				}
	...
	case TRADE_ITEM_OPEN_BORDERS:
		{
			...
			if (!pRenewDeal || !pRenewDeal->IsOpenBordersTrade(ePlayer))
			{
	...
	case TRADE_ITEM_DEFENSIVE_PACT:
		{
			...
			if (pRenewDeal && pRenewDeal->IsDefensivePactTrade(ePlayer))
			{
			...
			bool bAITradingWithHuman = !bHumanToHuman && pToPlayer->isHuman(ISHUMAN_AI_DIPLOMACY);
			...
			bAITradingWithHuman = !bHumanToHuman && pFromPlayer->isHuman(ISHUMAN_AI_DIPLOMACY);
	...
	case TRADE_ITEM_RESEARCH_AGREEMENT:
		{
			...
			// We already have a Research Agreement!
				...
				for (size_t i=0; i < vMembers.size(); i++)
				{
					...
					if (GET_PLAYER(vMembers[i]).isMinorCiv())
					{
						...
						for (size_t j=0; j < vFromTeam.size(); j++)
						{
							...
							if (!GET_PLAYER(vFromTeam[j]).isHuman(ISHUMAN_AI_DIPLOMACY) && GET_PLAYER(vFromTeam[j]).GetDiplomacyAI()->IsPeaceBlocked(vMembers[i]))
								return false;
						}
					}
					// Major civ?
	...
	case TRADE_ITEM_VOTE_COMMITMENT:
		{
			...
			if (!pFromPlayer->isHuman(ISHUMAN_AI_DIPLOMACY))
			{
			...
			if (!pToPlayer->isHuman(ISHUMAN_AI_DIPLOMACY))
			{
				// Is this the World Leader vote?
				if (pResolutionEffects->bDiplomaticVictory)
				{
...
bool CvDeal::BlockTemporaryForPermanentTrade(TradeableItems eItemType, PlayerTypes eFromPlayer, PlayerTypes eToPlayer)
{
	...
	bool bFromHuman = GET_PLAYER(eFromPlayer).isHuman(ISHUMAN_AI_DIPLOMACY);
	bool bToHuman = GET_PLAYER(eToPlayer).isHuman(ISHUMAN_AI_DIPLOMACY);
...
CvString CvDeal::GetReasonsItemUntradeable(PlayerTypes ePlayer, PlayerTypes eToPlayer, TradeableItems eItem, int iData1, int iData2, int iData3, bool bFlag1)
{
	...
	bool bPeaceDeal = pFromTeam->isAtWar(eToTeam);
	bool bFromHuman = pFromPlayer->isHuman(ISHUMAN_AI_DIPLOMACY);
	bool bToHuman = pToPlayer->isHuman(ISHUMAN_AI_DIPLOMACY);
	bool bSameTeam = eFromTeam == eToTeam;
...
void CvDeal::SetPeaceTreatyType(PeaceTreatyTypes eTreaty)
{
	...
	PRECONDITION(eTreaty < NUM_PEACE_TREATY_TYPES, "DEAL: Invalid PeaceTreatyType index.");
...
{
	PRECONDITION(ePlayer >= NO_PLAYER, "DEAL: Invalid Player Index.");	// NO_PLAYER is valid because we could be clearing the deal out for other uses
	PRECONDITION(ePlayer < MAX_CIV_PLAYERS, "DEAL: Invalid Player Index.");
...
void CvGameDeals::Init()
{
	...
	for (int i = 0; i < MAX_MAJOR_CIVS; i++)
	{
		...
		{
			m_aaiRenewDeal[i][j] = -1;
		}
	}
}

void CvGameDeals::SetRenewDealID(PlayerTypes ePlayer1, PlayerTypes ePlayer2, int iDealID)
{
	PRECONDITION(ePlayer1 >= 0 && ePlayer1 < MAX_MAJOR_CIVS, "Player index out of bounds");
	PRECONDITION(ePlayer2 >= 0 && ePlayer2 < MAX_MAJOR_CIVS, "Player index out of bounds");
	PRECONDITION(ePlayer1 != ePlayer2, "Other player must not be us");

	// Always use lower player ID as first index
	if (ePlayer1 > ePlayer2)
		std::swap(ePlayer1, ePlayer2);

	m_aaiRenewDeal[ePlayer1][ePlayer2] = iDealID;
}

int CvGameDeals::GetRenewDealID(PlayerTypes ePlayer1, PlayerTypes ePlayer2) const
{
	PRECONDITION(ePlayer1 >= 0 && ePlayer1 < MAX_MAJOR_CIVS, "Player index out of bounds");
	PRECONDITION(ePlayer2 >= 0 && ePlayer2 < MAX_MAJOR_CIVS, "Player index out of bounds");
	PRECONDITION(ePlayer1 != ePlayer2, "Other player must not be us");
	...
	return m_aaiRenewDeal[ePlayer1][ePlayer2];
}

CvDeal* CvGameDeals::GetRenewDeal(PlayerTypes ePlayer1, PlayerTypes ePlayer2)
{
	int iRenewDealID = GetRenewDealID(ePlayer1, ePlayer2);
	if (iRenewDealID > -1)
	{
		DealList::iterator it;
		for (it = m_CurrentDeals.begin(); it != m_CurrentDeals.end(); ++it)
		{
			if (it->m_iID == iRenewDealID)
				return &(*it);
		}
		ASSERT(false, "Renew Deal ID not found in list of current deals");
	}
	return NULL;
}

void CvGameDeals::ClearAllRenewDealIDs()
{
	for (int i = 0; i < MAX_MAJOR_CIVS; i++)
	{
		for (int j = 0; j < MAX_MAJOR_CIVS; j++)
...
void CvGameDeals::ActivateDeal(PlayerTypes eFromPlayer, PlayerTypes eToPlayer, CvDeal& kDeal, CvWeightedVector<TeamTypes>& veNowAtPeacePairs)
{
	...
	CvDeal* pRenewDeal = GC.getGame().GetGameDeals().GetRenewDeal(eFromPlayer, eToPlayer);
	if (pRenewDeal)
	{
		for (TradedItemList::iterator itemIterOldDeal = pRenewDeal->m_TradedItems.begin(); itemIterOldDeal != pRenewDeal->m_TradedItems.end(); ++itemIterOldDeal)
		{
			...
			GC.getGame().GetGameDeals().DoEndTradedItem(&*itemIterOldDeal, pRenewDeal->GetOtherPlayer(itemIterOldDeal->m_eFromPlayer), false);
		}
	}
	...
	for (TradedItemList::iterator it = kDeal.m_TradedItems.begin(); it != kDeal.m_TradedItems.end(); ++it)
	{
		...
		case TRADE_ITEM_GOLD_PER_TURN:
		{
			...
			if (pRenewDeal)
			{
				iGoldPerTurn -= pRenewDeal->GetGoldPerTurnTrade(eGivingPlayer);
			}
		...
		case TRADE_ITEM_RESOURCES:
		{
			...
			if (pRenewDeal)
			{
				iResourceQuantity -= pRenewDeal->GetNumResourceInDeal(eGivingPlayer, eResource);
			}
		...
		case TRADE_ITEM_THIRD_PARTY_PEACE:
		{
			...
			for (int iPlayerLoop = 0; iPlayerLoop < MAX_PLAYERS; iPlayerLoop++)
			{
				...
				if (eTeam == eTargetTeam)
				{
					...
					if (GET_PLAYER(ePlayer).isHuman(ISHUMAN_UI))
					{
		...
		case TRADE_ITEM_THIRD_PARTY_WAR:
		{
			...
			for (std::vector<PlayerTypes>::iterator iter = vPlayersWithSurveillance.begin(); iter != vPlayersWithSurveillance.end(); ++iter)
			{
				...
				if (!GET_PLAYER(*iter).isHuman(ISHUMAN_AI_DIPLOMACY))
				{
					if (!bCityState)
					{
	...
	if (pRenewDeal)
	{
		pRenewDeal->m_iFinalTurn = GC.getGame().getGameTurn() - 1;
	}
...
{//Cancel all proposed deals involving eCancelPlayer.
	...
	for(int iPlayerLoop = 0; iPlayerLoop < MAX_MAJOR_CIVS; iPlayerLoop++)
	{
		...
		if ((eTargetPlayers == DIPLO_AI_PLAYERS && !kLoopPlayer.isHuman(ISHUMAN_AI_DIPLOMACY)) || (eTargetPlayers == DIPLO_ALL_PLAYERS) || (eLoopPlayer == static_cast<PlayerTypes>(eTargetPlayers)))
		{
			CvDeal kDeal;
